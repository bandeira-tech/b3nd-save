/**
 * TYPE_TAGS recognition + JSON record encoding for Elasticsearch
 * entities.
 *
 * Documents are stored as JSON `_source` shapes. Per-field canonical
 * tags drive the JSON-boundary round-trip — bytes → base64, bigint →
 * string, timestamp → ISO-8601 — the same scheme used by
 * fs/s3/ipfs/localstorage so a record passes cleanly across stores.
 */
