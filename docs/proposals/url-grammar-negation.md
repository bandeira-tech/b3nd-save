...
globs match a *prefix* + a *tail glob*; there is no way to ask the inverse:
...
1. **Core stays a strict subset.** `b3nd-core`'s `compilePattern` (`?` literal,
   ...
   (v2 §7.1). Save may add grammar *on top* but must not require core to learn
...
2. **Push-down or honest fallback.** Every filter must either translate to each
   ...
   rule).
3. **No silent-default trap (§3.4).** A URL that *looks* like it filters but
...
- **Glob direction** — a new *token inside the URI path* (extends
  `splitLocatorGlob` + the three `glob.ts` compilers).
- **fn/param direction** — a new *query param or verb* (extends `ReadParams` +
...
- SQL (`globToSqlLike`): a single `LIKE` cannot negate a segment. Emit the
  ...
  predicate — i.e. the token must *lift out* of the LIKE body into a companion
...
Keep the positive glob as the *candidate set*; add a symmetric query param that
...
- `ReadParams.exclude?: string` in `src/url.ts`; parse it exactly like
  `pattern` (it is a glob, reuses all of `glob.ts`).
- `dispatchRead`: add `needsExcludePostFilter = exclude !== undefined &&
  !handlers.pushDownExclude`. In the post-filter block, after the `pattern`
  keep-filter, run a **drop**-filter with the same `compileSaveGlob(exclude)`
  regex (`arr.filter(uri => !re.test(tail))`).
...
| Push-down is a textbook `NOT LIKE` / `must_not` on every backend | Doesn't name "leaf" or "branch" — the *caller* encodes that distinction in the glob (`**/x` = leaf, `**/x/**` = branch). Easy to get subtly wrong |
...
*negated* form is the headline feature:
...
| + | − |
|---|---|
| **Directly answers the brief** — "leaf or branch" become named, discoverable params | Most new *concept* surface: two params × two polarities, plus validation rules |
...
Best when the leaf/branch distinction is the *primary* use case (which the brief
...
Best as an *escape hatch* for genuinely complex exclusions, not the everyday
...
| + | − |
|---|---|
| **Zero risk to the locked v2 grammar** — nothing reserved, fully experimental | `ext` is opaque and per-provider → *no uniform guarantee*, which is exactly the drift the grammar exists to prevent |
...
2. Add **C** as sugar over the same plumbing, with the fixed SQL/regex
   templates for real index use.
...
more than "negate a leaf or branch" needs) and **A** (breaks
`globToSqlLike`'s pure-body contract and reserves `!` in path segments — a
back-compat hazard for keys that legitimately contain `!`).

## Validation & edge rules (apply to whichever lands)

- **§3.4 no-silent-trap:** an `exclude`/`notLeaf`/`notBranch` on a URL with no
  `**`/`*` is still meaningful (filters a shallow `ls`); but an *unknown*
  negation param must throw via the existing `default:` in the `parseUrl`
  param switch (`src/url.ts:302`), never be ignored.
- **Empty result is content, not error** — consistent with "miss is
  `payload === undefined`".
- **Ordering:** negation filters run *after* `pattern` (positive narrowing)
  and *before* pagination/cursor — same slot as the current
  `needsPatternPostFilter` block so `limit`/`cursor` see the final set.
